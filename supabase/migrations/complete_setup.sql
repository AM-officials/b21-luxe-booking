...
-- 4. Franchise leads table for inquiry forms
...
DROP POLICY IF EXISTS "Allow read reviews to anon" ON public.google_reviews;
CREATE POLICY "Allow read reviews to anon"
    ON public.google_reviews FOR SELECT
    USING (true);

-- No insert/update/delete policies for google_reviews - service role only
